...
public class MyPQueueArray<T> extends MyPQueueBase<T> {
    ...
    public static void main(String[] args) {
        MyPQueueArray<Integer> pq = new MyPQueueArray<Integer>();
        int[] insert = {5, 3, 7, 1, 9, 2, 8, 4, 6, 0, 10, 11, 12};
        for (int x : insert) {
            pq.enque$raw(x);
            if (!pq.isValidHeap()) throw new AssertionError("heap invalid after insert " + x);
        }
        int last = Integer.MIN_VALUE;
        while (!pq.isEmpty()) {
            int v = pq.deque$raw();
            if (v < last) throw new AssertionError("deque order");
            last = v;
            if (!pq.isValidHeap()) throw new AssertionError("heap invalid after deque");
        }

        MyPQueueArray<String> pq2 = new MyPQueueArray<String>(
            java.util.Comparator.reverseOrder()
        );
        pq2.enque$raw("b");
        pq2.enque$raw("a");
        pq2.enque$raw("c");
        if (!"c".equals(pq2.top$raw())) throw new AssertionError("comparator top");
        System.out.println("MyPQueueArray checks ok");
    }
}
