...
public class Final_03 {
    ...
    static FnList<FnTupl2<FnList<Character>, Integer>> pg2701_word$count$listize3() {
        Final_03Map<Integer> map = new Final_03Map<Integer>();

        LnStrm<FnList<Character>> ws = Final_01.pg2701_word$strmize();
        LnStcn<FnList<Character>> c = ws.eval0();
        while (c.consq()) {
            String key = wordToString(c.hd());
            Integer prev = map.search$opt(key);
            map.insert$opt(key, prev == null ? 1 : prev + 1);
            c = c.tl().eval0();
        }

        MyRefer<FnList<FnTupl2<FnList<Character>, Integer>>> ref =
            new MyRefer<FnList<FnTupl2<FnList<Character>, Integer>>>(
                FnListSUtil.<FnTupl2<FnList<Character>, Integer>>nil()
            );
        map.foritm((k, v) -> {
            FnList<Character> word = stringToFnList(k);
            ref.set$raw(
                FnListSUtil.cons(
                    new FnTupl2<FnList<Character>, Integer>(word, v),
                    ref.get$raw()
                )
            );
        });
        FnList<FnTupl2<FnList<Character>, Integer>> WNS = ref.get$raw();
    ...
    public static void main(String[] args) {
        FnList<FnTupl2<FnList<Character>, Integer>> L = pg2701_word$count$listize3();
